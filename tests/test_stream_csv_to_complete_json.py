...
def test_stream_csv_to_complete_json():
    # Define the schema
    schema = StructType([
    ...
    upload_file('/reviews/data/in/reviews.csv', 'data/in/reviews.csv')

    # Define the input and output directories relative to the project root
    # input_dir = f'file:///{os.path.join(os.getcwd(), "data", "in")}'
    ...
    # -- LOAD
    query = top_rated_df.repartition(1).writeStream \
        .format("json") \
        .outputMode("append") \
